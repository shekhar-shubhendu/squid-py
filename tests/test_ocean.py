...
from squid_py.utils.utilities import Utilities
...
def test_split_signature():
    ...
    split_signature = utilities.split_signature(signature=signature)
...
def test_convert():
    utilities = Utilities(Web3)
    input_text = "my text"
    print("output %s" % utilities.convert_to_string(utilities.convert_to_bytes(input_text)))
    assert utilities.convert_to_text(utilities.convert_to_bytes(input_text)) == input_text
